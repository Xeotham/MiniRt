...
/*   Updated: 2024/06/20 16:28:14 by tde-la-r         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	create_amb_light(char **info, t_scene *scene)
{
	...
	if (ft_objfind_id(scene->light, ID_AMB_LIGHT) || ft_array_len(info) != 5)
		return (0);
...
int	create_point_light(char **info, t_scene *scene)
{
	...
	if (ft_objfind_id(scene->light, ID_LIGHT) || ft_array_len(info) != 5)
		return (0);
	obj = ft_calloc(sizeof(t_obj_list), 1);
	light = ft_calloc(sizeof(t_amb_light), 1);
	if (!obj || !light)
