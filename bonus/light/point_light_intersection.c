...
/*   Updated: 2024/08/23 15:23:10 by mhaouas          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include <minirt_bonus.h>

static t_color	specular_light(t_inter poi, t_vector3 poi_light_dir, t_point_light *light, t_vector3 camera_poi)
{
	...
	specular_color.d_red = (0.5 * pow(angle, 2) * light->ratio * light->color.red) / 255;
	specular_color.d_green = (0.5 * pow(angle, 2) * light->ratio * light->color.green) / 255;
	specular_color.d_blue = (0.5 * pow(angle, 2) * light->ratio * light->color.blue) / 255;
	return (specular_color);
}
...
t_color	compute_point_light(t_inter poi, t_ray poi_to_light, t_obj_list *lights, t_vector3 camera_pos)
{
	...
	specular_color = specular_light(poi, poi_to_light.dir, point_light, normalize_vector(vector_sub(poi.point, camera_pos)));
	diffuse_color.d_red = ft_dclamp(0, 1, diffuse_color.d_red + specular_color.d_red);
	diffuse_color.d_green = ft_dclamp(0, 1, diffuse_color.d_green + specular_color.d_green);
	diffuse_color.d_blue = ft_dclamp(0, 1, diffuse_color.d_blue + specular_color.d_blue);
	return (diffuse_color);
}
