...
static t_color	multi_light_color(t_inter poi, t_scene *scene, t_color base_color)
{
	t_ray			poi_to_light;
	t_color			color;
	...
	while (actual_light)
	{
		...
		if (!test_shadow(poi_to_light, poi, scene->objects))
			...
			color = compute_point_light(poi, poi_to_light , actual_light, scene->camera->orig);
		base_color.d_red = ft_dclamp(0, 1, base_color.d_red + color.d_red);
		base_color.d_green = ft_dclamp(0, 1, base_color.d_green + color.d_green);
		base_color.d_blue = ft_dclamp(0, 1, base_color.d_blue + color.d_blue);
